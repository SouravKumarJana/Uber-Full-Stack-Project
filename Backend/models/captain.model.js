const mongoose = require('mongoose');
const bcrypt = require('bcrypt');
const jwt = require('jsonwebtoken');


const captainSchema = new mongoose.Schema(
    {
        fullname:{
            firstname:{
                type: String,
                required: true,
                minlength: [3, "First name must be atleast 3 charchters long"]
            },
            lastname:{
                type: String,
                minlength: [6, "Last name must be atleast 6 charchters long"]
            }
        },    
        email:{
            type: String,
            required: true,
            lowercase: true,
            unique:true
//             match: [
// /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/,
// 'Please fill a valid email address'
// ]
        },
        password:{
            type: String,
            required: true,
            minlength: [5, "password must be atleast 5 charchters long"],
            select: false
        },
        sockedId:{
            type: String
        },
        status:{
            type: String,
            enum: ["active", "inactive"],
            default: "inactive"
        },
        vehicle:{
            color:{
                type: String,
                required: true,
                minlength: [3, "Color must be at least 3 characters long"]
            },
            plate:{
                type: String,
                required: true,
                minlength: [5, "vehicle plate number must be atleast 5 characters long"]
            },
            capacity:{      // How many passenger can carry the vehicle
                type: Number,
                required: true,
                min: [1, "capacity must be at least 1"]
            },
            vehicleType:{
                type: String,
                required: true,
                enum: ['car', 'motorcycle', 'auto']
            },
            model:{
                type: String,
                required: true,
                minlength: [4, "vehicle-model must be at least 4 characters long"]
            }
        },
        location:{
            latitude:{
                type: Number  // when the captain is inactive , then latitude is not required
            },
            longitude:{
                type: Number
            }
        }

    }
)

captainSchema.methods.generateAuthToken = function(){
    const token = jwt.sign({_id: this._id}, process.env.JWT_SECRET, {expiresIn: process.env.TOKEN_EXPIRY});
    return token;
}

captainSchema.statics.hashPassword = async function(password){
    return await bcrypt.hash(password, 10);
}

captainSchema.methods.comparePassword = async function(password){
    return await bcrypt.compare(password, this.password)
}

const captainModel = mongoose.model('captain', captainSchema);

module.exports = captainModel;